...
8. **`sales` (Takings):** One row per sale — invoice number, branch, what was sold, the price actually charged, how it was paid, the customer, the cashier, and the cost at the moment of sale so profit on a past sale does not move when a cost price is later corrected.
   - *Write-protected:* staff have `SELECT` only. Rows are created solely by the `record_sale` function, which moves the stock and records the money in one transaction and locks the unit (`FOR UPDATE`) so two cashiers cannot sell the same handset at the same second.
   - *Reversible, not erasable:* `void_sale` keeps the receipt, marks it `Voided` with a reason and who did it, and puts the stock back — a deleted receipt is how takings quietly stop adding up.
...
* **Selling from the phone:** A **Sell** tab that searches your stock as you type — model, IMEI, part name or SKU — instead of demanding the full identifier first, and lists what is actually sellable. The list price fills itself in and can be typed over for a discount; pick the payment method, name the customer if there is a warranty to honour, and confirm. The receipt shown is the stored one — real invoice number and timestamp — and the last sales stay on screen with a **Void** button that returns the stock.
...
* **Retail Cashier:** Pick the phone or accessory the customer is buying — every sellable item across all stores is in the one list, and the branch it leaves is read off the item, so a cashier is never staring at an empty dropdown because the register was set to the wrong store. Set the price (prefilled from the list price, editable for a discount), the payment method and who is selling, and confirming calls one database function that deducts the stock and writes the sale together. The receipt is printed from the stored row, so its invoice number and timestamp are the real ones. Below the form, the full sales history with the day's takings, and a **Void** on any sale that keeps the receipt and returns the stock.
