...
- **Glassmorphism**: Translucent cards, frosted glass sidebars, and soft backdrop blurs (ackdrop-filter).
...
### Core Mechanics
- **Dynamic Branches**: Stores are rows in a ranches table, so stores can be added, renamed, or archived dynamically without schema changes.
- **Strict Retail Sales Logic**: Cashiers are strictly prevented from selling stock that is not available in their currently selected branch (ecord_sale SQL transaction).
...
### 1. Android Mobile Application (1.13)
...
* **In-app updates:** The app checks the pp_releases table, downloads the new APK with a progress bar, and hands it to the Android package installer natively. 
...
1. **Install the app.** Copy uilds/app-debug.apk to the phone and install it (allow "install from unknown sources"). Or open the dashboard in a browser.
...
3. A ready-to-install debug build is committed at uilds/app-debug.apk. To rebuild it:
   `ash
   cd android-app
